...
# Полный прогон тест-плана:
#  1) Включить ЖМ и подтвердить по utcReplyFR (ретрай-цикл).
#  2) Удерживать ЖМ `hold1` секунд.
#  3) Вернуть штатный режим (LO=0, FF=0, mode=1), понаблюдать штатную программу `observe1` секунд.
#  4) Снова включить ЖМ (ретрай-цикл), удерживать `hold2` секунд.
#  5) Снова вернуть штатный режим, наблюдать `observe2` секунд.
#
# Скрипт снимает логи контроллера через tools/dk_capture.sh и сохраняет все под
...
# Базовая стратегия ЖМ: держать SetAF (utcControlFF=1) asserted >=10с (по требованиям UTMC),
# периодически переотправляя команду в течение окна, пока utcReplyFR не подтвердит.
YF_SEND_PERIOD_SEC=2
YF_WINDOW_SEC=90
YF_MIN_ASSERT_SEC=12

HOLD1_SEC=45
OBSERVE1_SEC=600   # 10 минут (2-3 цикла на этом ДК могут занимать ~7-11 минут)
HOLD2_SEC=60
OBSERVE2_SEC=120

PHASE_FALLBACK=1   # если прямые попытки не сработали: попробовать nominated phase 0x01 перед SET_YF
...
Опции:
  ...
  --max-attempts <n>        Попыток на этап включения ЖМ (по умолчанию: $MAX_ATTEMPTS)
  --confirm-sec <sec>       Окно подтверждения utcReplyFR (по умолчанию: $CONFIRM_SEC)
  --sleep-after-change <s>  Пауза после смены фазы перед SET_YF (по умолчанию: $SLEEP_AFTER_PHASE_CHANGE)
  --hold1-sec <sec>         Удержание первого ЖМ (по умолчанию: $HOLD1_SEC)
  --observe1-sec <sec>      Наблюдение штата после первого возврата (по умолчанию: $OBSERVE1_SEC)
  --hold2-sec <sec>         Удержание второго ЖМ (по умолчанию: $HOLD2_SEC)
  --observe2-sec <sec>      Наблюдение штата после второго возврата (по умолчанию: $OBSERVE2_SEC)
  --phase-fallback <0|1>    Включить фазовый fallback перед SET_YF (по умолчанию: $PHASE_FALLBACK)
EOF
      exit 0
      ;;
    *)
...
# Очистка (по возможности) даже при Ctrl+C.
cleanup() {
  set +e
  set +u
...
# OID (запросы выполняются локально на контроллере через 127.0.0.1)
...
send_set_phase_fallback() {
  remote_bash "
set -euo pipefail
# Выставить номинированную фазу (0x01) в UTC-режиме, затем попытаться включить ЖМ.
...
enable_yf_with_retry() {
  local label="$1"
  log "Стадия включения ЖМ: $label (окно=${YF_WINDOW_SEC}s период=${YF_SEND_PERIOD_SEC}s)"
  ...
  while [[ "$attempt" -le "$MAX_ATTEMPTS" ]]; do
    log "$label: попытка $attempt/$MAX_ATTEMPTS"
...
def parse_stat_samples(utmc_follow: Path):
    ...
    # Пример: '#15:16:05 STAT (137) ... 173 10 255 10 0 3 0 0 0$90'
