...
# Повторные попытки включить ЖМ (SET_YF) на контроллере до подтверждения по utcReplyFR,
# с параллельным снятием логов через tools/dk_capture.sh.
#
# Использование:
#   DK_PASS=... tools/yf_retry.sh [--ip 192.168.75.150] [--user voicelink] [--comm UTMC]
#
# Примечания:
# - Используется SSH, чтобы выполнять snmpget/snmpset локально на контроллере против 127.0.0.1.
# - Скрипт меняет состояние контроллера (ЖМ). Запускать только когда это безопасно.
...
Опции:
  ...
  --max-attempts <n>        Число попыток (по умолчанию: $MAX_ATTEMPTS)
  --confirm-sec <sec>       Окно подтверждения utcReplyFR после SET_YF (по умолчанию: $CONFIRM_SEC)
  --sleep-after-change <s>  Пауза после смены фазы перед SET_YF (по умолчанию: $SLEEP_AFTER_PHASE_CHANGE)
  --hold-sec <sec>          Удерживать ЖМ после успеха (по умолчанию: $HOLD_SEC)
EOF
      exit 0
      ;;
    *)
...
sshc() {
  # Держим вызовы короткими: sshpass кратковременно показывает пароль в списке процессов.
  sshpass -p "${DK_PASS}" ssh "${SSH_OPTS[@]}" "${USER}@${IP}" "$@"
}

remote_bash() {
  # Выполнить фрагмент bash-скрипта на контроллере (с экспортом переменных).
  local body="$1"
...
# OID (используются на контроллере через 127.0.0.1)
...
for i in $(seq 1 "$MAX_ATTEMPTS"); do
  ...
  # Каждую попытку начинаем из штатного режима (Standalone).
  remote_bash "
set -euo pipefail
snmpset -v1 -c \"\$COMM\" -Oqv 127.0.0.1 $OID_LO i 0 $OID_FF i 0 $OID_MODE i 1 >/dev/null
"
