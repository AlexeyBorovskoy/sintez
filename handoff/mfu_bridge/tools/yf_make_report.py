...
def parse_scenario_confirmations(scenario_text: str) -> list[tuple[str, str]]:
    """
    Извлекает строки CONFIRMED, которые пишет tools/yf_scripted_test.sh.
    Пример:
      [2026-02-18T11:25:20+03:00] CONFIRMED: utcReplyFR=1
    Возвращает список (timestamp, fr_value).
    """
...
def main() -> int:
    ...
    md = []
    md.append("# Отчет по сценарному тесту ЖМ\n")
    md.append(f"- Время формирования: `{dt.datetime.now().isoformat()}`\n")
    md.append(f"- Каталог прогона: `{run_dir}`\n")
    if base_dir:
        md.append(f"- База захвата: `{base_dir}`\n")
    md.append("\n## Конфигурация\n")
    md.append("```text\n")
    ...
    if scenario_confirms:
        ...
        md.append("- Подтверждения по сценарию: `не найдено`\n")
    ...
    md.append("```text\n" + ("\n".join(set_yf_lines) if set_yf_lines else "не найдено") + "\n```\n")
    md.append("\n### spectr_utmc_follow.log: >O.K.\n")
    md.append("```text\n" + ("\n".join(ok_lines) if ok_lines else "не найдено") + "\n```\n")
    md.append("\n### resident_follow.log: Remote mode\n")
    md.append("```text\n" + ("\n".join(remote_mode_lines) if remote_mode_lines else "не найдено") + "\n```\n")
    md.append("\n### resident_follow.log: Flashing yellow\n")
    md.append("```text\n" + ("\n".join(flashing_lines) if flashing_lines else "не найдено") + "\n```\n")
