# Spectr UTMC C++ Bridge

Мост между UTMC (Urban Traffic Management and Control) и Spectr-ITS для интеграции светофорных контроллеров.
...
- **snmp_handler.h/cpp** - Обработка SNMP операций (traps, GET, SET)
- **tcp_client.h/cpp** - TCP клиент с автопереподключением к ITS серверу
- **spectr_protocol.h/cpp** - Реализация протокола Spectr-ITS (checksum, форматирование команд/ответов)
...
- Формат: текстовые команды с checksum
...
Для отладки SNMP traps можно использовать:
```bash
