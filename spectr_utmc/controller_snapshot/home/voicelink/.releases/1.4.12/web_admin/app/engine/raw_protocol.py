...
class ServerPackage():
    ...
    def make_buffer(self):
        if sys.version_info.major == 3:
            ...
            #json_string = bytes(json.dumps(self.buffer_))
            if self.rpc == 10 and isinstance(self.buffer_, list):# log
                tmp = [i.decode('utf-8', 'replace').encode('utf-8', 'replace') if isinstance(i, str) else i for i in self.buffer_]
                json_string = bytes(json.dumps(tmp))
            else:
                json_string = bytes(json.dumps(self.buffer_))
