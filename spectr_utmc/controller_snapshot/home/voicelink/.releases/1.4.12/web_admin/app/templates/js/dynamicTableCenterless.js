                        ...
                        for (var i = 0; i < elements.length; i += 1) {
                            if (elements.item(i).id && elements.item(i).id === "status_link") {
                                if (ip && ans[ip][0] === 'check_ok') {
                                    elements.item(i).innerText = "{{ _('Connected') }}";
                                    elements.item(i).className = "label label-success";
                                } else {
                                    elements.item(i).innerText = "{{ _('Disconnected') }}";
                                    elements.item(i).className = "label label-danger";
                                }
                            } else if (elements.item(i).id && elements.item(i).id === "status") {
                                if (ip && ans[ip][1] === 'check_ok') {
                                    elements.item(i).innerText = "{{ _('Configured') }}";
                                    elements.item(i).className = "label label-success";
                                } else {
                                    elements.item(i).innerText = "{{ _('Not configured') }}";
                                    elements.item(i).className = "label label-danger";
                                }
                            }
                        }
            ...
            xhr.onreadystatechange = function() {
                ...
                for (var i = 0; i < tBody.rows.length; i += 1) {
                    ...
                    for (var j = 0; j < elements.length; j += 1) {
                        ...
                        {
                            if (ip) {
                                elements.item(j).innerText = "{{ _('Connected') }}";
                                elements.item(j).className = "label label-success";
                            } else {
                                elements.item(j).innerText = "{{ _('Disconnected') }}";
                                elements.item(j).className = "label label-danger";
                            }
                        } else if (elements.item(j).id &&
                            elements.item(j).id === "status")
                        {
                            if (ip && ans[ip].centerless_control.control_mode === "slave" && ans[ip].centerless_control.slave.master_ip === ans[ip].host) {
                                elements.item(j).innerText = "{{ _('Configured') }}";
                                elements.item(j).className = "label label-success";
                            } else {
                                elements.item(j).innerText = "{{ _('Not configured') }}";
                                elements.item(j).className = "label label-danger";
                            }
                        }
                    }
