...
def is_valid_ip(ip_addr):
    ...
    return bool(re.match(ip_regex, ip_addr))
...
def is_valid_hostname(hostname):
    ...
    hostname_regex = r'^([A-Za-z0-9]\.|[A-Za-z0-9][A-Za-z0-9-]{0,61}[A-Za-z0-9]\.){1,5}[A-Za-z]{2,6}$'
...
SYNC_TIMES = [
    ('10m', lazy_gettext('10 minutes')),
    ('30m', lazy_gettext('30 minutes')),
...
class DatetimeForm(Form):
    ...
    def validate_ntpserver(form, field):
        if field.data:
            if not is_valid_hostname(field.data) and not is_valid_ip(field.data):
                raise ValidationError("Invalid NTP server.")
