...
@app.route('/ignore_remote', methods=['GET', 'POST'])
@login_required
def ignore_remote():
    pages = {'controller_management': 'active'}
    if request.method == 'GET':
        st, data = rtcapi.readFromDev(rtcapi.CONFIG_ID_REMOTE_SCHEDULER)
        if st != rtcapi.RPC_OK:
            flash(gettext('Error! %(state)s', state = '{0}; {1};'.format(st, gettext(data).encode('utf8')).decode('utf8')), category='danger')
    elif request.method == 'POST':
        data = {}
        if request.form.get('onoff', False):
            data['remote_onoff'] = True
        req_times_start = {k.replace('starttime_', ''): v for k, v in request.form.to_dict().iteritems() if k.startswith('starttime_')}
        req_times_end = {k.replace('endtime_', ''): v for k, v in request.form.to_dict().iteritems() if k.startswith('endtime_')}
        schedule = []
        pattern = re.compile("^[1-7]$|^([1-7]\,){1,6}[1-7]{1}$")
        for i in sorted([int(x) for x in req_times_start.keys()]):
            req_days = request.form.to_dict().get('days_{}'.format(i), '')
            if req_days and pattern.match(req_days):
                schedule.append([req_times_start[str(i)], req_times_end[str(i)], req_days])
        data['remote_scheduler'] = schedule
        st, response = rtcapi.writeToDev(rtcapi.CONFIG_ID_REMOTE_SCHEDULER, data)
        if st != rtcapi.RPC_OK:
            flash(gettext('Error! %(state)s', state = '{0}; {1};'.format(st, gettext(str(response)).encode('utf8')).decode('utf8')), category='danger')
        return redirect(url_for('ignore_remote'))
    return render_template("ignore_remote.html",
        title=gettext('Remote scheduler'),
        data=data,
        pages=pages)
...
def current():
    ...
    if request.method == 'GET':
        ...
        prev_currents = session.get('prev_currents', None)
        if prev_currents:
            session.pop('prev_currents')
            current_lamps['currents'] = prev_currents
        if st != rtcapi.RPC_OK:
    ...
    if request.method == 'POST':
        ...
        req_currents = {k: v for k, v in request.form.to_dict().iteritems() if k.startswith('currents')}
        currents = {}
        
        # convertion from form data
        ...
        for gr in range(1, groups + 1):
            for n in range(0, 4):
                if str(gr) not in currents:
                    currents[str(gr)] = []
                curr = req_currents.get('currents_{}_{}'.format(gr, n))
                if curr is None:
                    if not missing_currents.get(gr):
                        missing_currents[gr] = []
                    missing_currents[gr].append(n)
                    currents[str(gr)].append(0)
                else:
                    currents[str(gr)].append(int(curr))

        # currents validation
        if missing_currents:
            error_message = '' 
            for gr, color in missing_currents.iteritems():
                error_message += ' ' + gettext('Group %(gr)s, channel %(color)s.', gr=gr, color=', '.join(str(x + 1) for x in color))
            flash(gettext('Error! %(state)s', state = '{0}; {1};'.format('', (gettext('Error saving group current settings.') + error_message).encode('utf8')).decode('utf8')), category='danger')
            session['prev_currents'] = currents
...
def localtime():
    ...
    if request.method == 'GET':
        ...
        if isinstance(datetimenow, dict):
            ...
            if last_sync_time:
                try:
                    ...
                    last_sync_time_obj = datetime.strptime(last_sync_time, '%Y-%m-%d %H:%M:%S')
        ...
        if form.validate_on_submit():
            ...
            st, message = rtcapi.writeToDev(rtcapi.CONFIG_ID_DATETIME, buf, 2)
            if st != rtcapi.RPC_OK:
                flash(gettext('Data saved.'), category='info')
                flash('{0}! {1}; {2}'.format(gettext('Error').encode('utf8'), st, gettext(message).encode('utf8')).decode('utf8'), category='danger')
        else:
            for form_error in form.errors:
                flash('{0}! {1}'.format(gettext('Error').encode('utf8'), gettext(''.join(form.errors[form_error])).encode('utf8')).decode('utf8'), category='danger')
...
def upload():
    ...
    if request.method == 'POST':
        ...
        st, allinfo = rtcapi.readFromDev(rtcapi.CONFIG_ID_GET_CURRENT_ALL_INFO)
        if allinfo and isinstance(allinfo, dict):
            program_latest_update = allinfo.get('program_latest_update', '')
        else:
            program_latest_update = None
...
def config_bakup():
    ...
    st, desc = rtcapi.readFromDev(rtcapi.CONFIG_ID_GET_DEVICE_INFO)
    if st != rtcapi.RPC_OK:
        flash(gettext('Error! %(state)s', state = '{0}; {1};'.format(st, desc).decode('utf8')), category='danger')
        desc = {}
    ...
    response.headers["Content-type"] ="multipart/form-data"
    response.headers["Content-Disposition"] = "attachment; filename={0}_config_bakup_{1}.tlc".format(desc.get('number', '').encode('utf-8'), datetime.strftime(datetime.now(), "%d.%m.%Y_%H.%M.%S"))
    return response
