...
function getAllValues(callback) {
    ...
    session.get(oids, (error, varbinds) => {
        ...
        for (const [key, oid] of Object.entries(OIDS)) {
            ...
            if (snmp.isVarbindError(varbind)) {
                const errMsg = snmp.varbindError(varbind);
                // Игнорируем NoSuchName ошибки (OID не существует)
                if (errMsg.includes('NoSuchName') || errMsg.includes('noSuchName')) {
                    result[key] = null; // OID не существует
                } else {
                    result[key] = { error: errMsg };
                }
            } else {
